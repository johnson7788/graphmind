...
前端"智能问答"页共 5 种检索模式，对应 LightRAG 的 `QueryParam.mode`：

| 界面标签 | mode | 检索方式 | 适用场景 |
|----------|------|----------|----------|
| 混合检索 | `mix` | **图谱 + 向量**：在 hybrid（图谱）基础上叠加向量文本块检索，最全面 | 通用默认，综合效果最佳 |
| 本地搜索 | `local` | **以实体为中心**：定位相关实体，取其描述及关联文本块 | 聚焦特定实体的具体问题 |
| 全局搜索 | `global` | **以关系为中心**：用高层关键词在关系/主题层聚合 | 宏观概括性问题 |
| 混合模式 | `hybrid` | **local + global**：同时做实体检索和关系检索并合并（均在图谱内） | 兼顾细节与全局 |
| 基础 RAG | `naive` | **纯向量检索**：不使用知识图谱，直接检索最相似的文本块 | 需要原始文本片段支撑 |

> **`mix` 与 `hybrid` 的区别**：`hybrid`（混合模式）= 本地 + 全局，两路都在**图谱内**；`mix`（混合检索）是 `hybrid` 的**超集**，在图谱检索之外再叠加一路**纯向量**文本块检索。二者不重复。

## 界面截图

| 图谱页面 | 图谱节点（charts 类型） | 混合搜索问答（含参考文档） |
|:---:|:---:|:---:|
| ![图谱页面](doc/图谱页面2.png) | ![图谱节点-charts类型](doc/图谱节点3-charts类型.png) | ![混合搜索问答](doc/混合搜索问答%20-给出参考文档.png) |
